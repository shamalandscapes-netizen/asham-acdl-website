...
import { Loader2, ArrowLeft, Package, ShieldCheck, ChevronRight } from 'lucide-react';
import Link from 'next/link';
import Image from 'next/image';
import { formatCurrency } from '@/lib/utils/formatters';
import { ProductPurchase } from '@/components/products/product-purchase';

// ✅ Fix: Receive params as a direct object, not a Promise
interface PageProps {
  params: {
    slug: string;
    id: string;
...
export default function ProductDetailPage({ params }: PageProps) {
  ...
  // Directly access the values from the object
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[60vh]">
      ...
      <p className="mt-4 text-xs font-black tracking-widest text-gray-400 uppercase">Loading Details...</p>
    </div>
  );
  ...
  if (!product) return (
    <div className="p-20 text-center">
      ...
      <Link href="/products" className="text-[#C75B39] font-bold underline mt-4 block">Return to Shop</Link>
    </div>
  );
  ...
  return (
    <div className="p-4 mx-auto max-w-7xl md:p-12">
      ...
      <nav className="flex items-center gap-2 mb-12 text-sm font-bold text-gray-400">
        <Link href="/products" className="hover:text-[#C75B39] transition-colors">Shop</Link>
        <ChevronRight size={14} />
        <Link href={`/products/${slug}`} className="capitalize hover:text-[#C75B39] transition-colors">
          {slug?.replace(/-/g, ' ')}
        </Link>
        <ChevronRight size={14} />
      ...
      <div className="grid grid-cols-1 gap-16 lg:grid-cols-2">
        ...
        <div className="relative aspect-square rounded-[3rem] overflow-hidden bg-white border border-gray-100 shadow-2xl p-8 group">
          <Image 
        ...
        <div className="flex flex-col justify-center">
          <div className="mb-6 space-y-2">
            ...
            <h1 className="text-5xl font-black text-[#06392F] leading-tight italic">{product.name}</h1>
          </div>
          ...
          <div className="py-8 mb-8 border-t border-gray-100">
            ...
            <p className="text-lg leading-relaxed text-gray-600">
              {product.description || 'Quality architectural plan or construction material sourced specifically for Asham ACDL clients.'}
            </p>
          </div>

          {/* Quantity Selector & Cart Logic */}
